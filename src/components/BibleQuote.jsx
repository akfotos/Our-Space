...
function BibleQuote() {
  ...
  return (
    <section className="glass-card p-6 animate-fade-in-up [animation-delay:0.05s]">
      <BookOpen
        size={56}
      ...
      <div className="relative flex flex-wrap gap-2 mb-4">
        {VERSIONS.map((v) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-semibold transition hover:scale-105 ${
              version === v.slug
                ? 'bg-gradient-to-br from-rose-500 to-rose-600 text-white shadow-md shadow-rose-500/25'
      ...
      <div className="relative bg-white/40 backdrop-blur-sm rounded-2xl p-5 border border-white/30 mb-4">
        {loading ? (
      ...
      <form onSubmit={fetchByReference} className="relative flex flex-col sm:flex-row gap-2">
        <input
          type="text"
          value={lookup}
          onChange={(e) => setLookup(e.target.value)}
          placeholder="John 3, John 3:16, or John 3:16-18"
          className="flex-1 rounded-xl border border-white/30 bg-white/40 px-4 py-2.5 text-slate-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-rose-300 transition focus:bg-white/60"
        />
        <button
          type="submit"
          disabled={!lookup.trim()}
          className="flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-700 hover:bg-slate-800 disabled:opacity-50 text-white rounded-xl font-medium transition hover:scale-105"
        >
          <Search size={16} />
          Lookup
        </button>
        <button
          ...
          className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white/40 hover:bg-white/60 text-slate-700 rounded-xl font-medium transition hover:scale-105"
        >
          <RefreshCw size={16} />
          Daily
        </button>
      ...
      {lookupError && (
        <p className="relative mt-2 text-sm text-red-600">{lookupError}</p>
      )}

      {items.length > 0 && (
        <div className="relative mt-5 space-y-3 max-h-64 overflow-y-auto scrollbar-thin pr-1">
          {items.map((item) => (
            <div
              key={item.id}
              className="bg-white/30 backdrop-blur-sm rounded-2xl p-4 border border-white/30 transition hover:bg-white/50"
            >
